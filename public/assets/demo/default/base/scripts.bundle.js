...
(function ($) {
	...
	$.fn.mDatatable = function (options) {
		...
		var dt = {
			...
			getData: function () {
				...
				if (options.data.type === 'remote') {
					...
					params.method = options.data.method || 'POST';
				}
		...
		var API = {
             ...
             * 获取某行某列的值
             */
            getColumnValue: function(index, columnName) {
                var tableBody = $(datatable.table).find('tbody')[0], value = '';
                $(tableBody).find('.m-datatable__cell:first-child').each(function (i, ceil) {
                    if (index == $(ceil).text()) {
                        datatable.API.row = $(ceil).closest('.m-datatable__row');
                        var rowNumber = datatable.API.row.index() + 1;
                        datatable.API.record = dt.getOneRow(tableBody, rowNumber); // 结果为 td 数组
                        datatable.API.record.each(function(j, td) {
                            if (columnName == $(td).attr('data-field')) {
                                value = datatable.API.value = $(td).text();
                                return false; // 结束循环
                            }
                        });
                        if (value) { // 如果获取到值，则返回false结束循环
                            return false;
                        }
                    }
                });
                return datatable.API.value;
            },
...
var mLayout = function() {
    ...
    var initQuicksearch = function() {
        ...
        qs.mQuicksearch({
            ...
            templates: {
                error: function(qs) {
                    return '<div class="m-search-results m-search-results--skin-light"><span class="m-search-result__message">好像坏掉了呢~</div></div>';
                }                            
            }
        });      
    }
